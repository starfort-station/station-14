...
petting-success-slimes = Вы погладили { THE($target) } на { POSS-ADJ($target) } слизистой поверхности.
...
petting-success-mimebot = Вы погладили { THE($target) } на { POSS-ADJ($target) } холодную металлическую голову.
...
petting-failure-mimebot = Вы протягиваете руку, чтобы погладить { THE($target) }, но { SUBJECT($target) } { CONJUGATE-BE($target) } занят мимикой!
